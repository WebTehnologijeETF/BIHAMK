	...
	ajax.onreadystatechange = function(){
		if(ajax.readyState == 4 && ajax.status == 200){
			...
			if(tab=="proizvodi.html")
				preuzmiPodatke();
			if(tab=="naslovnica.html")
				//dobaviIPrikaziNovosti();
				preuzmiNovosti();
			if(tab=="logout.php")
				location.reload();
		}
...
var prikaziDetaljno = function(index){
	var ajax = new XMLHttpRequest();
	...
	ajax.open("GET", "prikaziDetaljno.php?indeks=" + index, true);
	ajax.send();
}

var posaljiMail = function(){
	var formaZaKontakt = document.getElementById('forma');
	formaZaKontakt.action = "posaljiMail.php"; //will retrieve it
	formaZaKontakt.submit();
}

var unesiNovost = function(){
	var ajax = new XMLHttpRequest();
	...
	ajax.open("POST", "/phpskripte/novostiREST.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	
	var autor = document.getElementById("novostAutor").value; 
	var naslov = document.getElementById("novostNaslov").value;
	var slika = document.getElementById("novostSlika").value;
	var osnovniTekst = document.getElementById("naslovOsnovniTekst").value;
	var detaljniTekst = document.getElementById("naslovDetaljniTekst").value;

	ajax.send("autor=" + autor + "&naslov=" + naslov + "&slika=" + slika + "&osnovniTekst=" + osnovniTekst + "&detaljniTekst=" + detaljniTekst);
}

var obrisiNovost = function(idNovosti){
	var ajax = new XMLHttpRequest();
	...
	ajax.open("DELETE", "/phpskripte/novostiREST.php/" + idNovosti, true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	
	ajax.send();
}

var urediNovost = function(){
	var ajax = new XMLHttpRequest();
	
	ajax.onreadystatechange = function(){
		if(ajax.readyState == 4 && ajax.status == 200){
			console.log("Uspjesno: " + ajax.responseText);
			prikaziDetaljno(idNovosti);
		}
	...
	ajax.open("PUT", "/phpskripte/novostiREST.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	...
	var osnovniTekst = document.getElementById("naslovOsnovniTekst").value;
	var detaljniTekst = document.getElementById("naslovDetaljniTekst").value;
	var idNovosti = document.getElementById("idNovosti").value;

	ajax.send("autor=" + autor + "&naslov=" + naslov + "&slika=" + slika + "&osnovniTekst=" + osnovniTekst + "&detaljniTekst=" + detaljniTekst + "&idNovosti=" + idNovosti);
}

var obrisiKomentar = function(idKomentara){
	idNovosti = idKomentara.split(",")[1];
	idKomentara = idKomentara.split(",")[0];
	var ajax = new XMLHttpRequest();
	...
	ajax.open("DELETE", "/phpskripte/komentariREST.php/" + idKomentara, true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	
	ajax.send("id=" + idKomentara);
}

var unesiKomentarNaNovost = function(idNovosti){
	if(validirajUnosKomentara()){
		var ajax = new XMLHttpRequest();
		...
		ajax.open("POST", "/phpskripte/komentariREST.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		
		var autor = document.getElementById("komentarAutor").value; 
		var email = document.getElementById("komentarEmail").value;
		var tekst = document.getElementById("komentarTekst").value;
		
		ajax.send("autor=" + autor + "&email=" + email + "&tekst=" + tekst + "&idNovosti=" + idNovosti);
	}
}

var validirajUnosKomentara = function(){
	var autor = document.getElementById("komentarAutor"); 
	var tekst = document.getElementById("komentarTekst");

	if(autor.value == ""){
		autor.className = "nevalidnoPoljeForme";
		alert("Polje Autor ne smije biti prazno!");
		return false;
	}
	else{
		autor.className = "validnoPoljeForme";
	}
	if(tekst.value == ""){
		tekst.className = "nevalidnoPoljeForme";
		alert("Polje Tekst ne smije biti prazno!");
		return false;
	}
	else{
		tekst.className = "validnoPoljeForme";
	}
	return true;
} 

var posaljiLoginPodatke = function(){
	var ajax = new XMLHttpRequest();
	
	ajax.onreadystatechange = function(){
		if(ajax.readyState == 4 && ajax.status == 200){
			console.log("Uspjesno: " + ajax.responseText);
			if(!(ajax.responseText.substring(0,34) == "Username ili Password nije validan")){
				location.reload();
			}
			else{
				sadrzaj.innerHTML = ajax.responseText;	
	...
	ajax.send("username=" + username + "&password=" + password + "&submit=true");
}

var validirajUnosKorisnika = function(){
	var usernameKorisnika = document.getElementById("korisnikUsername").value; 
	var passwordKorisnika = document.getElementById("korisnikPassword").value;
	var emailKorisnika = document.getElementById("korisnikEmail").value;

	if(usernameKorisnika.value == ""){
		usernameKorisnika.className = "nevalidnoPoljeForme";
		alert("Polje Username ne smije biti prazno!");
		return false;
	}
	else{
		usernameKorisnika.className = "validnoPoljeForme";
	}
	if(passwordKorisnika.value == ""){
		passwordKorisnika.className = "nevalidnoPoljeForme";
		alert("Polje Password ne smije biti prazno!");
		return false;
	}
	else{
		passwordKorisnika.className = "validnoPoljeForme";
	}
	if(emailKorisnika.value == ""){
		emailKorisnika.className = "nevalidnoPoljeForme";
		alert("Polje Email ne smije biti prazno!");
		return false;
	}
	else{
		emailKorisnika.className = "validnoPoljeForme";
	}
	return true;
} 

var unesiKorisnika = function(){
	if(validirajUnosKorisnika()){
		...
		ajax.open("POST", "/unesiKorisnika.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		...
		ajax.send("username=" + usernameKorisnika + "&password=" + passwordKorisnika + "&email=" + emailKorisnika);
	}
}

var obrisiKorisnika = function(usernameKorisnika){
	var ajax = new XMLHttpRequest();
	...
	ajax.open("POST", "/obrisiKorisnika.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	
	ajax.send("username=" + usernameKorisnika);
}

var napraviId =  function(){
    var tekst = "";
    var rjecnik = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";

    for( var i=0; i < 5; i++ )
        tekst += rjecnik.charAt(Math.floor(Math.random() * rjecnik.length));

    return tekst;
}


var generisiNovost = function(novostObjekat){
	var novostKontejnerDiv = document.createElement('div');
	novostKontejnerDiv.className = "novostKontejner";
	novostKontejnerDiv.id = napraviId();

	var idNovostiInput = document.createElement('input');
	idNovostiInput.type = "hidden";
	idNovostiInput.value = novostObjekat["id"];
	novostKontejnerDiv.appendChild(idNovostiInput);

	var naslovNovostiDiv = document.createElement('div');
	naslovNovostiDiv.className = "naslovNovosti";
	naslovNovostiDiv.innerHTML = novostObjekat["naslov"];
	novostKontejnerDiv.appendChild(naslovNovostiDiv);

	var autorIDatumNovostiDiv = document.createElement('div');
	autorIDatumNovostiDiv.className = "autorIDatumNovosti";
	autorIDatumNovostiDiv.innerHTML = novostObjekat["autor"] + " - " + novostObjekat["datum"];
	novostKontejnerDiv.appendChild(autorIDatumNovostiDiv);

	var slikaNovostiDiv = document.createElement('div');
	slikaNovostiDiv.className = "slikaNovosti";
	var slikaNovostiImg = document.createElement('img');
	slikaNovostiImg.src = novostObjekat["slika"];
	slikaNovostiImg.height = 250;
	slikaNovostiImg.width = 250;
	slikaNovostiDiv.appendChild(slikaNovostiImg);
	novostKontejnerDiv.appendChild(slikaNovostiDiv);

	var osnovniTekstDiv = document.createElement('div');
	osnovniTekstDiv.className = "osnovniTekstNovosti";
	osnovniTekstDiv.innerHTML = novostObjekat["osnovniTekst"];
	novostKontejnerDiv.appendChild(osnovniTekstDiv);

	if(novostObjekat["detaljniTekst"] != null){
		var detaljniTekstDiv = document.createElement('div');
		detaljniTekstDiv.className = "detaljniTekstNovosti";
		detaljniTekstDiv.innerHTML = novostObjekat["detaljniTekst"];
		//detaljniTekstDiv.style.display = "none";
/* 
		var detaljnoDugmeDiv = document.createElement('div');
		detaljnoDugmeDiv.className = "detaljniTekstNovosti";

		var detaljnoDugmeLink = document.createElement('a');
		detaljnoDugmeLink.className = "dugmeDetalji";
		detaljnoDugmeLink.href = "#" + novostKontejnerDiv.id;


		detaljnoDugmeLink.innerHTML = "Detaljnije...";
		detaljnoDugmeDiv.appendChild(detaljnoDugmeLink);

		var sakrijDugmeDiv = document.createElement('div');
		sakrijDugmeDiv.className = "detaljniTekstNovosti";

		var sakrijDugmeLink = document.createElement('a');
		sakrijDugmeLink.className = "dugmeDetalji";
		sakrijDugmeLink.href = "#" + novostKontejnerDiv.id;

		sakrijDugmeLink.innerHTML = "Detaljnije...";
		sakrijDugmeDiv.appendChild(detaljnoDugmeLink);

		
		novostKontejnerDiv.appendChild(detaljnoDugmeDiv);
		novostKontejnerDiv.appendChild(sakrijDugmeDiv);
		*/
		novostKontejnerDiv.appendChild(detaljniTekstDiv);
	}

	if(novostObjekat["komentari"] != null){
		var komentariKontejnerDiv = document.createElement('div');
		komentariKontejnerDiv.className = "novostKontejner";

		var komentariNaslov = document.createElement('div');
		komentariNaslov.className = "naslovNovosti";
		komentariNaslov.style.fontSize = "smaller";
		komentariNaslov.innerHTML = "Komentari(" + novostObjekat["komentari"].length + ")";
		komentariKontejnerDiv.appendChild(komentariNaslov);

		for (var i = 0; i < novostObjekat["komentari"].length; i++) {
			var komentariAutorIDatum = document.createElement('div');
			komentariAutorIDatum.className = "autorIDatumNovosti";
			komentariAutorIDatum.innerHTML = novostObjekat["komentari"][i]["autor"] + " - " + novostObjekat["komentari"][i]["datum"];

			if(novostObjekat["komentari"][i]["email"] != null){
				komentariAutorIDatum.innerHTML += " - ";

				var komentariEmail = document.createElement('a');
				komentariEmail.href = "mailto:" + novostObjekat["komentari"][i]["email"];
				komentariEmail.innerHTML = novostObjekat["komentari"][i]["email"];

				komentariAutorIDatum.appendChild(komentariEmail);
			}

			var komentariTekst = document.createElement('div');
			komentariTekst.className = "osnovniTekstNovosti";
			komentariTekst.innerHTML = novostObjekat["komentari"][i]["tekst"];	

			komentariKontejnerDiv.appendChild(komentariAutorIDatum);
			komentariKontejnerDiv.appendChild(komentariTekst);
		}
		novostKontejnerDiv.appendChild(komentariKontejnerDiv);
	}

	return novostKontejnerDiv;	
}

var dobaviIPrikaziNovosti = function(){
	var ajax = new XMLHttpRequest();
	...
	ajax.onreadystatechange = function(){
		if(ajax.readyState == 4 && ajax.status == 200){	
			...
			for (var i = 0; i < response.novosti.length; i++) {
				novostiHTML += generisiNovost(response.novosti[i]).outerHTML;
			};

			sadrzaj.innerHTML = novostiHTML;
			//return new response.novost;
		}
	...
	ajax.open("GET", "/phpskripte/novostiREST.php/", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	
	ajax.send();
}

var dobaviNovost = function(idNovosti){
	var ajax = new XMLHttpRequest();
	
	if(idNovosti == null || idNovosti == undefined || !idNovosti){
		idNovosti = "";
	}
	...
	ajax.open("GET", "/phpskripte/novostiREST.php/" + idNovosti, true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	
	ajax.send();
}
/*
var prikaziDetaljniTekst = function(idVijesti){
	var novost = document.getElementById(idVijesti);
	novost.getElementsByClassName("detaljniTekstNovosti")[0].display = "block";
	novost.getElementsByClassName("dugmeDetalji")[0].display = "none";
	novost.getElementsByClassName("dugmeDetalji")[1].display = "block";
}

var sakrijDetaljniTekst = function(idVijesti){
	var novost = document.getElementById(idVijesti);
	novost.children.getElementsByClassName("detaljniTekstNovosti")[0].display = "none";
	novost.children.getElementsByClassName("dugmeDetalji")[0].display = "none";
	novost.children.getElementsByClassName("dugmeDetalji")[1].display = "block";

}
*/

// Logika za reload stranice ukoliko je user neaktivan jednu minutu
var idleTime = 0;
